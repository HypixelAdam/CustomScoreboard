...
public class CsAliasesHelp implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String l, String[] a) {
		...
		int length = a.length;
		if (cmd.getName().equalsIgnoreCase("cshelpaliases")) {
			if (!p.hasPermission("customscoreboard.command.helpaliases")) {
				p.sendMessage(format(prefix + "&cYou don't have permission to run this command."));
				return true;
			}
			if (length == 0) {
				hm.sendNormalAliasesHelp(p);
				return true;
			} else if (length == 1) {
